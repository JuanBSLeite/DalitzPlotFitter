...
def boost_to_rest_frame(momentum: Array, frame_momentum: Array) -> Array:
    ...
    boosted_spatial = jnp.where((beta2 > 0.0)[..., None], boosted_spatial, spatial)
...
def covariant_kinematics(
    ...
    resonance_mass = jnp.sqrt(jnp.maximum(invariant_mass_squared(resonance), 0.0))
    ...
    cos_theta = jnp.sum(
        daughter_resonance[..., 1:] * bachelor_resonance[..., 1:], axis=-1
    ) / safe_denominator
