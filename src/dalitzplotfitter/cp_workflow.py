...
class CPFitSession:
    ...
    def from_root(
        ...
        **session_kwargs,
        ...
        """Create a simultaneous CP session directly from two ROOT TTrees."""
    ...
    def with_efficiency(
        self,
        plus_efficiency: object | None,
        minus_efficiency: object | None = None,
    ) -> "CPFitSession":
        return replace(
            self,
            plus_efficiency=plus_efficiency,
            minus_efficiency=plus_efficiency if minus_efficiency is None else minus_efficiency,
        )

    def with_veto(
        self,
        plus_veto: object | None,
        minus_veto: object | None = None,
    ) -> "CPFitSession":
        return replace(
            self,
            plus_veto=plus_veto,
            minus_veto=plus_veto if minus_veto is None else minus_veto,
        )
    ...
    def with_background(
        ...
        *,
        ...
        spec = CPBackgroundSpec(
            name=name,
            plus_shape=plus_shape,
            minus_shape=minus_shape,
            fraction=fraction,
            yield_=yield_,
            plus_normalization_sample=plus_normalization_sample,
            minus_normalization_sample=minus_normalization_sample,
    ...
    @cached_property
    def plus_cache(self):
        return self.plus_model.prepare_cache(
            self.plus_data,
            self.plus_model.normalization_sample,
            efficiency_normalization=self.plus_acceptance_normalization,
        )

    @cached_property
    def minus_cache(self):
        return self.minus_model.prepare_cache(
            self.minus_data,
            self.minus_model.normalization_sample,
            efficiency_normalization=self.minus_acceptance_normalization,
        )
    ...
    def _build_background(
        self, background: CPBackgroundSpec | CPBackgroundCategory
        ...
        plus_norm = (
            self.plus_model.normalization_sample
            if background.plus_normalization_sample is None
            else background.plus_normalization_sample
        )
        minus_norm = (
            self.minus_model.normalization_sample
            if background.minus_normalization_sample is None
            else background.minus_normalization_sample
        )
        plus_shape = background.plus_shape
        minus_shape = background.resolved_minus_shape

        plus_data_dict = self.plus_data.as_dict()
        minus_data_dict = self.minus_data.as_dict()
        plus_norm_dict = plus_norm.as_dict()
        minus_norm_dict = minus_norm.as_dict()
        ...
        if background.apply_veto:
            if self.plus_veto is not None:
                plus_values = plus_values * jnp.asarray(self.plus_veto(plus_data_dict))
                plus_norm_values = plus_norm_values * jnp.asarray(self.plus_veto(plus_norm_dict))
            if self.minus_veto is not None:
                minus_values = minus_values * jnp.asarray(self.minus_veto(minus_data_dict))
                minus_norm_values = minus_norm_values * jnp.asarray(self.minus_veto(minus_norm_dict))
        ...
        return CPBackgroundCategory(
            name=background.name,
            plus_values=plus_values,
            minus_values=minus_values,
            plus_normalization=plus_normalization,
            minus_normalization=minus_normalization,
    ...
    def base_objective(self) -> CPJointNLL:
        return CPJointNLL(
            self.plus_cache,
            self.minus_cache,
            plus_efficiency=self.plus_acceptance_data,
            minus_efficiency=self.minus_acceptance_data,
            background_categories=self.background_categories,
    ...
    def print_fit_fractions(
        ...
        *,
        ...
        values = self.result_values(result)
        plus_eff = self.plus_efficiency if acceptance_weighted else None
        minus_eff = self.minus_efficiency if acceptance_weighted else None
    ...
    def report(
        ...
        *,
        ...
        """Return and print a compact CP fit report."""
        ...
        report: dict[str, object] = {
            ...
            "values": values,
            "errors": errors,
            "charge_probabilities": {"plus": float(pplus), "minus": float(pminus)},
        }
    ...
    def _projection_components(
        ...
        acceptance = (
            self.plus_acceptance_normalization
            if charge == "plus"
        ...
        norm_intensity = jnp.asarray(model.intensity(norm.as_dict(), values))
        signal_density = acceptance * norm_intensity / joint_signal_norm
        ...
        for index, (source, category, scale) in enumerate(
            ...
            bg_norm_sample = (
                (source.plus_normalization_sample or self.plus_model.normalization_sample)
                if charge == "plus"
                else (source.minus_normalization_sample or self.minus_model.normalization_sample)
            )
            ...
            joint_norm = category.normalization
            density = raw / joint_norm
            components.append(
                (
                    category.name,
                    bg_norm_sample,
