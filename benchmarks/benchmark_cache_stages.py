...
This benchmark is intentionally separate from ``benchmark_fit_evaluation.py``.
It measures the first coefficient-only normalization and data-side XLA programs
without mixing their timings with Minuit or likelihood compilation.
...
def main() -> None:
    ...
    lowered_norm, norm_lower_seconds = _timed(
        lambda: normalization_kernel.lower(norm_data, weights, efficiency)
    )
    compiled_norm, norm_compile_seconds = _timed(lowered_norm.compile)
    norm_outputs, norm_first_execute_seconds = _timed_blocking(
        lambda: compiled_norm(norm_data, weights, efficiency)
    )
    ...
    for _ in range(args.repeats):
        ...
        norm_warm_times.append(elapsed)
    ...
    matrix_host = np.asarray(jax.device_get(fixed_matrix))
    components_host = np.asarray(jax.device_get(data_components))
    ...
    payload = {
        ...
        "normalization_lower_seconds": norm_lower_seconds,
        "normalization_compile_seconds": norm_compile_seconds,
        "normalization_first_execute_seconds": norm_first_execute_seconds,
        "normalization_warm_execute_seconds_mean": float(np.mean(norm_warm_times)),
        "normalization_warm_execute_seconds_min": float(np.min(norm_warm_times)),
        "normalization_warm_execute_seconds_max": float(np.max(norm_warm_times)),
        ...
        "data_warm_execute_seconds_min": float(np.min(data_warm_times)),
        "data_warm_execute_seconds_max": float(np.max(data_warm_times)),
        "normalization_matrix_diagonal_min": float(np.min(diagonal_host)),
        "normalization_matrix_diagonal_max": float(np.max(diagonal_host)),
        "normalization_matrix_finite": bool(np.all(np.isfinite(matrix_host))),
        "data_components_finite": bool(np.all(np.isfinite(components_host))),
    }
