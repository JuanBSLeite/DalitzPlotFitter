...
def _event():
    ...
    return phase_space.from_unit_square(jnp.asarray([[0.41, 0.37]]))
...
def test_boost_to_resonance_rest_frame_zeroes_resonance_momentum():
    ...
    assert jnp.allclose(boosted[..., 1:], 0.0, atol=1e-12, rtol=0.0)
    assert jnp.allclose(
        boosted[..., 0] ** 2,
...
def test_covariant_momenta_match_two_body_invariant_formulae():
    ...
    p_star_expected = math.sqrt(
        kallen(m_parent**2, m_res**2, m_bachelor**2)
    ) / (2 * m_parent)
    ...
    assert math.isclose(float(values.p_star[0]), p_star_expected, rel_tol=1e-11)
    assert math.isclose(float(values.p[0]), p_expected, rel_tol=1e-11)
    assert math.isclose(float(values.q[0]), q_expected, rel_tol=1e-11)
...
def test_equal_mass_daughter_exchange_flips_covariant_helicity_angle():
    ...
    assert jnp.allclose(first.resonance_mass, second.resonance_mass, atol=1e-12)
    assert jnp.allclose(first.p_star, second.p_star, atol=1e-12)
    assert jnp.allclose(first.p, second.p, atol=1e-12)
    assert jnp.allclose(first.q, second.q, atol=1e-12)
    assert jnp.allclose(first.cos_theta, -second.cos_theta, atol=1e-11)
